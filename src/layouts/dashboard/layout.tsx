...
export function DashboardLayout({ sx, children, data }: DashboardLayoutProps) {
  ...
  const getNavDataFinal = (data: any, role: string) => {
    const finalNav: any = [];
    data.forEach((nav: any) => {
      const finalItems: any = [];
      for (let i = 0; i < nav.items.length; i++) {
        if (nav.items[i].roles.includes(role)) finalItems.push(nav.items[i]);
      }
      nav.items = finalItems;
      if (finalItems.length) finalNav.push(nav);
    });
    return finalNav;
  };

  const navDataFinal = getNavDataFinal(navData, user?.user?.role);
...
function useNavColorVars(theme: Theme, settings: SettingsState): Record {
  const {
    vars: { palette },
  } = theme;
